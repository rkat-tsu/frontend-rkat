...
export default function Edit({ auth, unit, users, units, approvalPaths }) {
    ...
    const submit = (e) => {
        ...
        toast.warning("Konfirmasi Simpan", {
            description: "Apakah Anda yakin ingin menyimpan perubahan unit kerja ini?",
            action: {
                label: "Ya, Simpan",
                onClick: () => {
                    const toastId = toast.loading("Sedang memperbarui data...");
                    patch(route('unit.update', unit.uuid || unit.id_unit), {
                        onSuccess: () => toast.success("Berhasil", { id: toastId, description: `Data Unit Kerja ${data.nama_unit} berhasil diperbarui.` }),
